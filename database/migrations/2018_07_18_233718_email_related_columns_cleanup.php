...
class EmailRelatedColumnsCleanup extends Migration
{
    /**
     * Run the migrations.
     */
    ...
    public function down()
    {
        ...
        DB::statement('UPDATE `accounts` AS `a` 
                           INNER JOIN (SELECT `account_id`, 
                                          ...
                                          `enable_email_markup`, 
                                          `email_footer` 
                                       FROM   `account_email_settings`) AS `aes` 
                           ON `aes`.`account_id` = `a`.`id` 
                        SET `a`.`enable_reminder1` = `aes`.`enable_reminder1`, 
                            `a`.`enable_reminder2` = `aes`.`enable_reminder2`, 
                            `a`.`enable_reminder3` = `aes`.`enable_reminder3`, 
                            `a`.`enable_reminder4` = `aes`.`enable_reminder4`, 
                            `a`.`num_days_reminder1` = `aes`.`num_days_reminder1`, 
                            `a`.`num_days_reminder2` = `aes`.`num_days_reminder2`, 
                            `a`.`num_days_reminder3` = `aes`.`num_days_reminder3`, 
                            `a`.`direction_reminder1` = `aes`.`direction_reminder1`, 
                            `a`.`direction_reminder2` = `aes`.`direction_reminder2`, 
                            `a`.`direction_reminder3` = `aes`.`direction_reminder3`, 
                            `a`.`field_reminder1` = `aes`.`field_reminder1`, 
                            `a`.`field_reminder2` = `aes`.`field_reminder2`, 
                            `a`.`field_reminder3` = `aes`.`field_reminder3`, 
                            `a`.`email_design_id` = `aes`.`email_design_id`, 
                            `a`.`enable_email_markup` = `aes`.`enable_email_markup`, 
                            `a`.`email_footer` = `aes`.`email_footer`
        ');
    }
}
