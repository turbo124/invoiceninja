...
class ClientBalanceReportOptimizationTest extends TestCase
{
    ...
    public function testQueryCountReduction()
    {
        ...
        // Current implementation: 2 queries per client (count + sum)
        // Expected after optimization: ~2 queries total
        $expectedLegacyQueries = $clientCount * 2;
        $this->assertGreaterThan($expectedLegacyQueries * 0.8, $queryCount,
            "Expected ~{$expectedLegacyQueries} queries for {$clientCount} clients");
    }
    ...
    public function testLargeDatasetPerformance()
    {
        ...
        // Current: 200 queries (100 clients × 2 queries)
        // After optimization: ~2 queries
        $this->assertGreaterThan(150, $queryCount, 'Legacy should make ~200 queries');
        $this->assertNotEmpty($output);
