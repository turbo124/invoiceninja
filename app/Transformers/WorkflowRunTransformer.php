...
class WorkflowRunTransformer extends EntityTransformer
{
    ...
    public function transform(WorkflowRun $run)
    {
        return [
            ...
            'entity_type' => (string) class_basename($run->workflowable_type),
            'entity_id' => (string) $this->encodePrimaryKey($run->workflowable_id),
            'entity_hashed_id' => (string) $this->encodePrimaryKey($run->workflowable_id),
