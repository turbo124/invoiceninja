...
class Statics
{
    ...
    public static function company($locale = false): array
    {
        ...
        if ($locale) {
            $data['industries'] = app('industries')->each(function ($industry) {
                $industry->name = ctrans('texts.industry_'.$industry->name);
            })->sortBy(function ($industry) {
                return $industry->name;
            })->values();

            $data['countries'] = app('countries')->each(function ($country) {
                $country->name = ctrans('texts.country_'.$country->name);
            })->sortBy(function ($country) {
                return $country->name;
            })->values();

            $data['payment_types'] = app('payment_types')->each(function ($pType) {
                $pType->name = ctrans('texts.payment_type_'.$pType->name);
            })->sortBy(function ($pType) {
                return $pType->name;
            })->values();

            $data['languages'] = app('languages')->each(function ($lang) {
                $lang->name = ctrans('texts.lang_'.$lang->name);
            })->sortBy(function ($lang) {
                return $lang->name;
            })->values();

            $data['currencies'] = app('currencies')->each(function ($currency) {
